...
class Patient:
    ...
    def INSERT_PAT(self):
        ...
        cur.execute("use hello")
        p=[]
        cur.execute("SELECT * FROM PAT WHERE PAT_ID =%s",(p1,))
        for i in cur:
            p.append(i)
        x=len(p)
        if x!=0:
            ...
            cur.execute('INSERT INTO PAT VALUES(%s,%s,%s,%s,%s,%s,%s,%s)',(p1,p2,p3,p4,p5,p8,p9,p10,))
            cur.execute('INSERT INTO CONTACT VALUES (%s,%s,%s)',(p1,p6,p7,))
    ...
    def UPDATE_PAT(self):
        ...
        cur.execute("use hello")
        p=[]
        cur.execute("SELECT * FROM PAT WHERE PAT_ID =%s",(u1,))
        for i in cur:
            p.append(i)
        if len(p) != 0:
            cur.execute('UPDATE PAT SET NAME=%s,SEX=%s,DOB=%s,BLOOD_GRP=%s,ADDRESS=%s,CONSULT=%s,EMAIL=%s where PAT_ID=%s', ( u2, u3, u4, u5, u10, u9, u8,u1,))
            cur.execute('UPDATE CONTACT set CONTACT_NO=%s,ALT_CONTACT=%s WHERE PAT_ID=%s', ( u6, u7,u1,))
...
class DMenu:
    ...
    def DELETE_PAT(self):        
        ...
        cur.execute("use hello")
        p=[]
        cur.execute("SELECT * FROM PAT WHERE PAT_ID =%s",(inp_d,))
        for i in cur:
            p.append(i)
        if (len(p)==0):
...
class SMenu:
    ...
    def SEARCH_PAT(self):
        ...
        cur.execute("use hello")
        p=[]
        cur.execute("SELECT * FROM PAT WHERE PAT_ID =%s",(inp_s,))
        for i in cur:
            p.append(i)            
        if (len(p)==0):
            tkinter.messagebox.showerror("HOSPITAL DATABSE SYSTEM","PATIENT RECORD NOT FOUND")
            ...
            t=[]
            cur.execute("SELECT * FROM PAT NATURAL JOIN CONTACT WHERE PAT_ID =%s",(inp_s,))
            for j in cur:
                t.append(j) 
            print(t)
            for i in t:
