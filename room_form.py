...
class Room:
    ...
    def __init__(self,master):
        ...
        self.room_t1= Label(self.LoginFrame,text="ROOM TYPE",font="Helvetica 14 bold",bg="cadet blue",bd=22)
        ...
        self.lblda=Label(self.LoginFrame,text="DATE ADMITTED(YYYY-MM-DD)",font="Helvetica 14 bold",bg="cadet blue",bd=22)
        ...
        self.lbldd=Label(self.LoginFrame,text="DATE DISCHARGED(YYYY-MM-DD)",font="Helvetica 14 bold",bg="cadet blue",bd=22)
    ...
    def INSERT_ROOM(self):
        ...
        r6=(self.dd.get())
        p = []
        cur.execute("SELECT * FROM ROOM WHERE ROOM_NO=%s",(r3,))
        for i in cur:
            p.append(i)
        x=len(p)
        if x!=0:
            ...
            cur.execute('INSERT INTO ROOM VALUES(%s,%s,%s,%s,%s,%s)',(r1,r3, r2, r4, r5, r6,))
    ...
    def UPDATE_ROOM(self):
        global P_id,r1,r2,room_t,da,dd,rate,room_no,r3,r4,r5,r6
        con=sq.connect(host="localhost",user="root",password="alanwalker")
        cur=con.cursor()
        cur=con.cursor(buffered=True)
        cur.execute("create database if not exists hello")
        ...
        r6=(self.dd.get())
        p = []
        cur.execute("Select * from ROOM where PAT_ID=%s AND ROOM_NO=%s",(r1,r3,))
        for i in cur:
            p.append(i)
        if len(p) == 0:
            ...
            cur.execute('UPDATE ROOM SET ROOM_NO=%s,ROOM_TYPE=%s,RATE=%s,DATE_ADMITTED=%s,DATE_DISCHARGED=%s where PAT_ID=%s',(r3, r2, r4, r5, r6,r1,))
...
class S_Room:
    ...
    def ROOM_DISPLAY(self):
        ...
        p=[]
        cur.execute('select * from  ROOM  where PAT_ID=%s',(pat_rm,))
        for i in cur:
            p.append(i)
        if (len(p)==0):
